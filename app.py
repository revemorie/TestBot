...
def webhook():
  ...
  if data['text'].lower() == "!cheeto":
  ...
  elif data['text'].lower() == "!birth":
    msg = "Happy Birthday!"
    send_message_picture(msg,birthArray)
    ...
    for i in range(10):
      if int(toPrint[i][0]) != 0:
        msg+=str(toPrint[i][1])+" received "+str(toPrint[i][0])+" likes in the past 25000 messages\n"
  ...
  elif data['text'].lower() == "!alltimeleaderboardavg":
    ...
    leaderboardName.append('CheetoBot')
    leaderboardNumber=[0]*len(leaderboardId)
    leaderboardMsgNum=[0]*len(leaderboardId)
    msgId=json.loads(r.text)['response']['messages']['last_message_id']
    ...
    for i in range(100):
      ...
      for j in messages:
        ...
        if j['sender_type']=='user' or j['sender_type']=='bot':
          ...
          user=j['user_id']
          if user in leaderboardId:
            leaderboardMsgNum[leaderboardId.index(user)]+=1
            for k in likes:
              leaderboardNumber[leaderboardId.index(user)]+=1
  ...
  elif ("i’m" in data['text'].lower() or "i'm" in data['text'].lower() ) and data['sender_type']!='bot' and ((time.time())-countTime)>3600 and False:
    countTime=time.time()
